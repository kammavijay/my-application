...
function validate()
{
   ...
   if(temp.loginname.value=="" || temp.noofbags.value=="" || temp.bagname.value=="")
   {
...
<tr>
  ...
  <%} %>
  <form action="BookinBags.jsp" name="register" onsubmit="return validate()">
<table>
    <tr>
		...
		<%  FlightBooking fbook =new FlightBooking();
                CoreHash fbookcorehash= fbook.getBookingDetails((String)session.getAttribute("user"));
                Set<Integer> kys5 = fbookcorehash.keySet();
                Integer getid = 0;
                ...
                for(Integer i5:kys5)
                {
                    Integer fbookbean = (Integer)fbookcorehash.get(i5);
                %><option value="<%=fbookbean%>" <%=(fbookbean.equals(getid)?"selected":"") %>><%=fbookbean%></option><%
                }
            %>
		  </select>
        </label></td>
        ...
        <td><label>
		  ...
		  <%  AirlineName airlinenames=new AirlineName();
                ...
                for(Integer i1:kys1)
                {
                    ...
                    %><option value="<%=airlinesname%>" <%=(airlinesname.equals(getan)?"selected":"") %>><%=airlinesname%></option><%
                }
            %>
		  </select>
        </label></td>
      </tr>
      <tr><td>LoginName</td><td>:<input type="text" name="loginname" value="<%=(String)session.getAttribute("user")%>"/></td></tr>
      <tr><td>NoofBags</td><td>:<input type="text" name="noofbags"/></td></tr>
      <tr><td>BagName</td><td>:<input type="text" name="bagname"/></td></tr></table>
      <input type="submit" name="send" value="Bagage"/>
</form>
