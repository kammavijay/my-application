...
<%@ page import="java.util.*,com.dts.core.util.*,com.dts.crmofairlines.model.*,com.dts.crmofairlines.dao.*" %>
<html><head>
...
<tr>
  ...
  <%} %>
  <form action="CustomerProcessServiceHistroy.jsp">
  <%
  ServiceHistroy aservicehistroy=new ServiceHistroy();
  CoreHash acorehash=aservicehistroy.viewAirLines();
  Enumeration aenumeration=acorehash.keys();
 %><table>
 <tr><td>AirLinesName</td><td><select name="airlinesname">
 <%
  ...
  Service aservice=(Service)acorehash.get(i1);
   %>
<option value="<%=aservice.getAirlinename()%>"><%=aservice.getAirlinename()%></option><%} %></select></td></tr>
</table><input type="submit" name="ServiceAirlines" value="ServiceHistroy"/></form>
